...
mongoose.connect(process.env.MONGO_URI)
    .then(() => console.log('Connected to MongoDB (Hospital A)'))
...
app.put('/api/patients/:id', async (req, res) => {
    try {
        ...
        if (updatedPatient.shared) {
            ...
            const apiKey = process.env.MEDICONNECT_API_KEY || 'sk_test_12345hospitalA';
...
app.post('/api/share', async (req, res) => {
    try {
        ...
        // Use bulk endpoint — single HTTP round-trip for all patients
        const bulkUrl = (process.env.MEDICONNECT_URL || 'http://localhost:5000/api/ingest-hospital-data')
            .replace('/api/ingest-hospital-data', '/api/ingest-hospital-data/bulk');
        const apiKey = process.env.MEDICONNECT_API_KEY || 'mediconnect-secret-key';

        const payload = patients.map(p => p.toObject());
        const response = await axios.post(bulkUrl, payload, {
            headers: { 'x-api-key': apiKey, 'Content-Type': 'application/json' }
        });

        // Count successes from MediConnect response
        const results = response.data?.results || [];
        const successIds = patients
            .filter((_, i) => results[i]?.ok)
            .map(p => p.id);
        const successCount = successIds.length;

        // Bulk-mark all successfully shared patients in one DB call
        if (successIds.length > 0) {
            await Patient.updateMany({ id: { $in: successIds } }, { $set: { shared: true } });
        }

        res.json({
            message: `Successfully shared ${successCount} out of ${patients.length} selected patients with MediConnectAI.`
        });
    } catch (error) {
...
app.put('/api/removal-requests/:id/approve', async (req, res) => {
    try {
        ...
        try {
            // Fetch patient to include national_id for Mediconnect merged-record lookup
...
// Receive rejection ping from MediConnect — mark local removal request as Rejected
app.post('/api/webhook/reject', async (req, res) => {
    try {
        const { patientId } = req.body;
        if (!patientId) {
            return res.status(400).json({ error: 'patientId is required' });
        ...
        // Mark all pending removal requests for this patient as Rejected
        const result = await RemovalRequest.updateMany(
            { patientId: patientId, status: { $in: ['Pending Hospital Approval', 'Pending MediConnect Approval'] } },
            { $set: { status: 'Rejected', updatedAt: Date.now() } }
        );

        console.log(`Rejection webhook: marked ${result.modifiedCount} request(s) as Rejected for patient ${patientId}`);
        res.json({ message: `Removal request(s) for patient ${patientId} marked as Rejected.` });
    } catch (error) {
        console.error('Error handling reject webhook:', error);
        res.status(500).json({ error: 'Internal server error handling reject webhook' });
    }
});

app.listen(PORT, () => {
    console.log(`Hospital A Backend running on port ${PORT}`);
});
